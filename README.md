...
```
├── data/
│   └── 10_years_data.csv          # Daily CMT Treasury yields (FRED)
├── models/
...
│   └── nelson_siegel.py           # Nelson-Siegel-Svensson parametric fit
└── main.py
```
...
- Parametric curve fit: $y(\tau) = \beta_0 + \beta_1 \frac{1 - e^{-\lambda\tau}}{\lambda\tau} + \beta_2\left(\frac{1-e^{-\lambda\tau}}{\lambda\tau} - e^{-\lambda\tau}\right)$
- Parameters: level ($\beta_0$), slope ($\beta_1$), curvature ($\beta_2$), decay ($\lambda$)
- Rolling calibration shows how yield curve shape evolves across rate regimes

### Phase 3 — PCA of the Yield Curve
- PCA on daily yield curve changes $\Delta y_t(\tau)$ across all 11 CMT tenors
...
$$r_{t+\Delta t} | r_t \sim c \cdot \chi^2(d, \lambda), \quad c = \frac{\sigma^2(1-e^{-a\Delta t})}{4a}, \quad d = \frac{4ab}{\sigma^2}, \quad \lambda = \frac{4ae^{-a\Delta t}}{\sigma^2(1-e^{-a\Delta t})}r_t$$
